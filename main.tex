...
To make these approaches comparable, we introduce a compact taxonomy that highlights the main axes along which game-planning methods differ.
\paragraph{Taxonomy.}
We structure the survey using three dimensions that recur across game-planning methods:
\begin{itemize}
    \item \textbf{Representation:} symbolic structures (operators/HTNs/cases), continuous scores (utilities), learned world models, or natural-language abstractions.
    \item \textbf{Deliberation mechanism:} explicit plan synthesis/decomposition, action scoring/selection, or simulation-based lookahead (search).
    \item \textbf{Design integration:} real-time feasibility, robustness under uncertainty, and the level of designer control and interpretability.
\end{itemize}
...
\subsection{Decision-Making Paradigms in Game AI}

\subsubsection{Reactive control}
...
\subsubsection{Planning-based deliberation}
...
\subsubsection{Utility-based action selection}
...
Together, these paradigms illustrate that contemporary planning in games is increasingly hybrid in nature, combining symbolic structure, search, learning, and language-based reasoning. Understanding their respective trade-offs provides essential context for the comparative analysis presented in this section.
